...
SWITCH (TRUE) {
	...
	CASE (@$url[$n-1] == 'basket' && @$url[$n] == 'add'):
		...
		$product=$db->get_single("SELECT id,parent,name,IF(price_sale > 0, price_sale, price) `price`,sale,article,
	...
	CASE ($url[$n]=='basket' && count($url)==2):
			...
			foreach ($_POST['edit_number'] as $k=>$v) {
				for ($i=0;$i<count($_SESSION['fw_basket']);$i++) {
					if ($_SESSION['fw_basket'][$i]['id']==$k && preg_match("/^([0-9]+)$/",$v)) { 
							$_SESSION['fw_basket'][$i]['number']=preg_replace("/\D/", "", $v);
					}
				}
			}
		...
		if (count($_SESSION['fw_basket'])>0) {
			$total_price=0;
			for ($i=0;$i<count($_SESSION['fw_basket']);$i++) {
				if (isset($_SESSION['fw_basket'][$i]['price'])) {
					$total_price+=$_SESSION['fw_basket'][$i]['price']*$_SESSION['fw_basket'][$i]['number'];
				}
			}
        ...
        foreach($sess['fw_basket'] as $key=>$val){
        	foreach($sess['fw_basket'][$key] as $key2=>$val2)
        		if (isset($sess['fw_basket'][$key]['price'])) {
        			$sess['fw_basket'][$key]['price_number'] = sprintf("%.2f",$sess['fw_basket'][$key]['price']*$sess['fw_basket'][$key]['number']);
        			$sess['fw_basket'][$key]['full_url'] = DOMAIN . 'catalog' . $shop->getFullUrlProduct($sess['fw_basket'][$key]['id']);
        			$sess['fw_basket'][$key]['info'] = $shop->getProductInfo($sess['fw_basket'][$key]['id']);
		...
		if (isset($_POST['submit_order']))
		{
			...
			else
			{
				...
				for ($i=0;$i<count($_SESSION['fw_basket']);$i++)
				{
					...
					if (empty($_SESSION['fw_basket'][$i]['id'])) {
						continue;
					}
			...
			$order_price = 0;
				...
				$db->query("INSERT INTO fw_orders (
					...
					'".$user['mail']."','".$user['address']."',
						...
						if ($_SESSION['fw_basket'])
						{
						    ...
						    $products = array();
      						foreach ($_SESSION['fw_basket'] as $key=>$val)
      						{
      							  ...
      							  $products[$key]['full_url'] = DOMAIN . $shop->getFullUrlProduct($val['id'], 'catalog');
      						}
						...
						$user['mail'] = preg_replace("/[\t\n]/"," ",$user['mail']);
						
						if (preg_match("/([a-z0-9\.\-\@\_]+)\@([^\b]+)/is", $user['mail'], $match)) {
						   $user['mail'] = trim($match[1].'@'.$match[2]);
						}
						
						Mail::send_mail($user['mail'], "Vlasovtoys@yandex.ru","Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
	
						//письмо администратору сайта
						$admin_body=$smarty->fetch($templates_path.'/admin_order_notice.txt');
							
						$subject = "=?windows-1251?b?" . base64_encode("Новый заказ в интернет магазине #{$order_id}") . "?=";
						$to = "=?windows-1251?B?" . base64_encode("Vlasovtoys@yandex.ru") . "?= <" . "Vlasovtoys@yandex.ru" . ">";
						$headers = "From: =?windows-1251?B?" . base64_encode($user['mail']) . "?= <{$user['mail']}>\n";
						$headers .= "Content-Type:text/html\n";
						@mail($to, $subject, $admin_body, $headers);

						setcookie("catalog_user", $user['name']."|" . $user['mail'] . "|" . $user['address'], time() + 60*60*24*30*12, "/", ".".$_SERVER['SERVER_NAME'] );

						header("Location: /basket_final/");
						die();
			}
	...
	CASE (isset($url[1]) && $url[1] == 'search'):
		...
		if (!empty($_GET['query']))
		{
			$query = htmlspecialchars(strip_tags($_GET['query']), ENT_QUOTES);
			$query = preg_replace("/(select|delete|remove|drop|alter|revoke|truncate|create|dump|insert|update)/is", "", $query);
		...
		$products = $shop->search($where, $sort, $order);
		...
		$smarty->assign("products_list",$products);
		$page_found=true;
