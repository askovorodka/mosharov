...
if (isset($_POST['submit_add_cat'])) {
    ...
    $sale = !empty($_POST['edit_cat_sale']) ? $_POST['edit_cat_sale'] : 0;
...
if (isset($_POST['submit_edit_cat'])) {
    ...
    if ($check || $id=="1") {
        ...
        $db->query("UPDATE fw_catalogue SET name='$name', sale='$sale', image='$image',title='$title',text='$text',url='$url',status='$status',meta_keywords='$keywords',meta_description='$description', show_in_menu = '{$show_in_menu}' WHERE id='$id'");

        if (preg_match("/^([0-9\,\.]+)$/is", $sale)) {
            $sale_number = number_format(($sale/100), 1, ".", "");
            $db->query("UPDATE fw_products SET price_sale = (price-(price * {$sale_number})) WHERE parent='{$id}'");
        }
...
if (isset($_POST['submit_add_product'])) {
    ...
    $price_sale=String::secure_format($_POST['edit_price_sale']);
    ...
    $db->query("INSERT INTO fw_products 
		...
		article,parent,name,
		title,site_url,
		price,price_sale,price2,insert_date,
...
if (isset($_POST['submit_add_photo'])) {
    ...
    if ($check) {
        ...
        if (move_uploaded_file($tmp, BASE_PATH."/uploaded_files/shop_images/$id.$ext")) {
            ...
            Image::resize(BASE_PATH."/uploaded_files/shop_images/$id.$ext", BASE_PATH."/uploaded_files/shop_images/800-$id.$ext", 800,800, false, "#FFFFFF");
            Image::resize(BASE_PATH."/uploaded_files/shop_images/$id.$ext", BASE_PATH."/uploaded_files/shop_images/1024-$id.$ext", 1024,1024, false, "#FFFFFF");
            ...
            //Image::image_resize(BASE_PATH."/uploaded_files/shop_images/$id.$ext", BASE_PATH."/uploaded_files/shop_images/800-$id.$ext", 800,800);
            //Image::image_resize(BASE_PATH."/uploaded_files/shop_images/$id.$ext", BASE_PATH."/uploaded_files/shop_images/1024-$id.$ext", 1024,1024);
...
if (isset($_POST['submit_number_recount'])) {
    ...
    $db->query("UPDATE fw_orders SET total_price=(SELECT SUM(product_count * product_price) FROM fw_orders_products WHERE order_id='$order_id') WHERE id='$order_id'");
    $location=$_SERVER['HTTP_REFERER'];
    header("Location: $location");
}
...
// SORRY for so strange separator
    ...
    CASE ($action=='orders'):
        ...
        $orders_list=$db->get_all("
			SELECT id,user,status,status as status_number,insert_date,total_price,
    ...
    CASE ($action=='mini_catalogue' && isset($_GET['order_id'])):
        ...
        if (isset($_GET['cat'])) {
            ...
            $cat=$_GET['cat'];
            $result=$db->query("SELECT COUNT(*) FROM fw_products WHERE parent='$cat' AND status='1'");
            $pager=Common::pager($result,PRODUCTS_PER_PAGE,$page);
