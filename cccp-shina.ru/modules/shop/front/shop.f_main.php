...
if (isset($_GET['update_price_from_itire']))
{
	...
	else
	{
		...
		if ($_POST['type'] == 'tire')
		{
			...
			$tire_koef = $_POST['cccp_tire_koef'];
			$tire_round_koef = $_POST['cccp_tire_round_koef'];
		...
		elseif ($_POST['type'] == 'disk')
		{
			...
			$disk_koef = $_POST['cccp_disk_koef'];
			$disk_round_koef = $_POST['cccp_disk_round_koef'];
...
//					}
	...
	CASE ( ($url[$n]=='basket' && count($url)==2) || (preg_match("/\?ver=(.+)$/",$url[$n]) && $url[$n-1] == 'basket' && count($url)==3) || (preg_match("/\?error=(.+)$/",$url[$n]) && $url[$n-1] == 'basket' && count($url)==3)):
	...
	CASE ($url[$n]=='submit' && $url[$n-1]=='basket' && count($url)==3):
		...
		if (isset($_POST['submit_order'])) 
		{
			...
			else
			{
				...
				Mail::send_mail($user['mail'],"cccp-tire@yandex.ru","Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
