...
$string = new String();
//$id = $tree->clear();
//$tree->insert($id, array("name" => "1", "url" => "1"));
/*
$id = $tree->clear();
$tree->insert($id, array("name" => "����", "url" => "tyres"));
$tree->insert($id, array("name" => "�����", "url" => "rims"));
*/
	...
	if ($check) {
		...
		if (@$file_name!='') {
			...
			if (move_uploaded_file($tmp, BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext)) {
				...
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext,BASE_PATH.'/uploaded_files/shop_images/bcat-'.$id.'.'.$ext,$details['width'],$details['height'], false);
