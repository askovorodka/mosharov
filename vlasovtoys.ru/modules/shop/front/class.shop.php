...
class Shop extends db {
	...
	function search($where, $sort_field = "date", $sort_order = "desc")
	{
		if (count($where) > 0)
		{
			$where = " and " . implode(" and ", $where);
		...
		$query = "select a.*, b.image,
			(SELECT id FROM fw_products_images i WHERE i.parent=a.id ORDER BY sort_order ASC LIMIT 1) AS image,
			(SELECT ext FROM fw_products_images WHERE parent=a.id ORDER BY insert_date DESC LIMIT 1) AS ext
			from fw_products as a
			left join fw_catalogue as b on a.parent = b.id 
			where a.status = '1' " . 
	...
	{
		$result = $this->db->get_single("
			SELECT a.id, a.name, a.description, a.parent, IF(a.price_sale > 0, a.price_sale, a.price) `price`,
			(SELECT id FROM fw_products_images i WHERE i.parent=a.id ORDER BY sort_order ASC LIMIT 1) AS image,
			(SELECT ext FROM fw_products_images WHERE parent=a.id ORDER BY insert_date DESC LIMIT 1) AS ext
			FROM fw_products as a
