        ...
        CASE ($url[$n]=='index.xml' && count($url)==2):
            ...
            if (count($products_list)>0) {
                ...
                for ($a=0;$a<count($products_list);$a++) {
					...
					//params
                    /*$colors = $shop->getProductPropertiesByEntity($products_list[$a]['id'], PROPERTY_ENTITY_COLOR);
                    if (!empty($colors)){
                        $products_list[$a]['colors'] = $colors;
                    }
                    $sizes = $shop->getProductPropertiesByEntity($products_list[$a]['id'], PROPERTY_ENTITY_SIZE);
                    if (!empty($sizes)) {
                        $products_list[$a]['sizes'] = $sizes;
                    }
					print_r(PROPERTY_ENTITY_COLOR);
                    print_r(PROPERTY_ENTITY_SIZE);*/


                    $productProperties = $shop->getProductProperties($products_list[$a]['id'], 'size', '0', 1);
                            ...
                            if (!empty($property['colors']))
                            {
                                ...
                                foreach($property['colors'] as $key2=>$color)
                                {
                                    ...
                                    if (!in_array($color['value'], $returnProperties['sizes'][$property['value']])){
                                        ...
                                        $returnProperties['sizes'][$property['value']] = $property['value'];
                                        $returnProperties['sizes_brand'][$brandKey] = $brandKey;
                                    }
                                ...
                                $returnProperties['colorsNames'] = $colorNames;
                                //$returnProperties['sizes'] = array_keys($returnProperties['sizes']);
                            }
                        }
