...
if ($action == "delete_size" && isset($_GET['id']))
{
	$id = (int) $_GET['id'];
	if (!empty($id))
	{
		$db->query("delete from `properties` where `parent_id`='{$id}' ");
...
SWITCH (TRUE) {
	...
	CASE ($action == 'colors'):
		...
		$navigation[]=array("url" => BASE_URL."/admin/?mod=shop&action=colors","title" => 'Цвета');
	...
	CASE ($action == 'promo_codes'):
		...
		$navigation[]=array("url" => BASE_URL."/admin/?mod=shop&action=promo_codes","title" => 'Промо коды');
...
/****************************************************************/
		...
		$product_properties_old = $db->get_all("select * from fw_products_properties where product_id='$id'");
		...
		$smarty->assign('product_properties_old',$product_properties_old);
