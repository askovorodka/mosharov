...
if (isset($_POST['submit_restore']))
{
	...
	if (isset($_POST['email']))
	{
      ...
      if ($item['id'] != "")
	  {
            ...
            $subject = "Регистрационные данные http://" . $_SERVER['SERVER_NAME'];
            $message = 'Ваш логин: ' . $item['login'] . '<br>Ваш пароль: ' .$password;

            $headers  = "Content-type: text/html; charset=windows-1251 \r\n";
            $headers .= "From: <noreply@".$_SERVER['SERVER_NAME'].".ru>\r\n";

		  	$send = mail($_POST['email'], $subject, $message, $headers);
            if ($send){
				$smarty->assign('message', 'Новый пароль отправлен на почту');
			}
      }
...
if (isset($_POST['submit_user_register'])) {
	...
	$lastname=String::secure_user_input(iconv('utf-8','windows-1251',$_POST['userlastname']));
	...
	$city=String::secure_user_input(iconv('utf-8','windows-1251',$_POST['city']));
	$street=String::secure_user_input(iconv('utf-8','windows-1251',$_POST['street']));
	$house=String::secure_user_input(iconv('utf-8','windows-1251',$_POST['house']));
	$post_index=String::secure_user_input(iconv('utf-8','windows-1251',$_POST['index']));
	...
	if ($registration_success==false)
	{
		...
		$db->query("INSERT INTO fw_users(login, password, name, lastname, mail, tel, reg_date, priv, status, address, city, street, house, post_index) VALUES('{$login}','".sha1($password)."','{$name}','{$lastname}','{$login}','{$phone}','".time()."','9','1', '{$address}', '{$city}', '{$street}', '{$house}', '{$post_index}')");
...
if (isset($_POST['submit_edit_user'])) {
	...
	$lastname=String::secure_user_input($_POST['lastname']);
...
SWITCH (TRUE){
	...
	CASE ($url[$n] == "save" && count($url) == 2):
		if (!empty($_POST))
		{
			...
			$lastname = trim($_POST['cart_lastname']);
			$phone_1 = trim($_POST['cart_phone']);
			$address = trim($_POST['cart_address']);
			$city = trim($_POST['cart_city']);
			$street = trim($_POST['cart_street']);
			$house = trim($_POST['cart_house']);
			$post_index = trim($_POST['cart_index']);
			$delivery = $_POST['cart_delivery'];
			$info = $_POST['cart_info'];


			$db->query("update fw_users set `name`='{$name}', `lastname`='{$lastname}', `phone_1`='{$phone_1}', `address`='{$address}', `city`='{$city}', `street`='{$street}', `house`='{$house}', `post_index`='{$post_index}', `delivery`='{$delivery}', `info`='{$info}' where id='{$userId}' ");
