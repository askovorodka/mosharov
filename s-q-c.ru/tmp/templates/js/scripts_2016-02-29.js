(function () {
 ...
 //выбираем цвета
    /*$('.item__info__selecter .color').click(function(event){
        ...
        $('.item__info__selecter .color').removeClass('g-hidden');

        if ($(event.target).hasClass('selected')) {return;}

        var color = $(event.target).attr('data-color');
        var colorSizes = properties.colors[color]; //it is array

        $('.item__info__selecter .size').each(function(index, elem){
            if (colorSizes.indexOf($(elem).attr('data-size')) == -1) {
    ...
    $('.item__info__selecter .size').click(function(event){
		...
		if ($('#item-sizetype').val() == 'rus') {
			var sizeColors = properties.sizes[size]; //it is array
		} else {
			var sizeColors = properties.sizes_brand[size]; //it is array
		}
		
        $('.item__info__selecter .color').each(function(index, elem){
            if (sizeColors.indexOf($(elem).attr('data-color')) == -1) {
                $(elem).addClass('g-hidden');
            }
        });
        event.stopPropagation();
