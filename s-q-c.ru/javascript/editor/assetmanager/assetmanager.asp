...
if Not IsEmpty(oFolder_base0) then
	if InStr(CStr(currFolder),CStr(oFolder_base0.path))<>0 AND bReadOnly0=true then bWriteFolderAdmin=true
end if
if Not IsEmpty(oFolder_base1) then
	if InStr(CStr(currFolder),CStr(oFolder_base1.path))<>0 AND bReadOnly1=true then bWriteFolderAdmin=true
end if	
if Not IsEmpty(oFolder_base2) then
	if InStr(CStr(currFolder),CStr(oFolder_base2.path))<>0 AND bReadOnly2=true then bWriteFolderAdmin=true
end if	
if Not IsEmpty(oFolder_base3) then
	if InStr(CStr(currFolder),CStr(oFolder_base3.path))<>0 AND bReadOnly3=true then bWriteFolderAdmin=true
end if	
sFolderAdmin=""
...
function writeFileSelections()
	...
	'Response.Write "<tr><td colspan=4 width='100%'><b>"&getText("Files")&"</b></td></tr>"
	sColor = "#e7e7e7"
	for each oFile in oFiles
		...
		if bDisplay then
			...
			Response.Write "<tr style='background:" & sColorResult & "'>" & VbCrLf & _
				...
				"<td valign=top style='cursor:pointer;' onclick=""selectFile(" & nIndex & ")"" width=100% ><u id=""idFile"&nIndex&""">" & oFile.name & "</u></td>" & VbCrLf & _
				...
				"<td valign=top nowrap onclick=""deleteFile(" & nIndex & ")""><u style='font-size:10px;cursor:pointer;color:crimson' " & sFolderAdmin & "><script>document.write(getText('del'))</script></u></td>" & VbCrLf & _
				"</tr>" & VbCrLf	
		end if
	next

	if bFileFound=false then
		Response.Write "<tr><td colspan=4 height=100% align=center><script>document.write(getText('Empty...'))</script></td></tr></table></div>"
	else
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<td>
				...
				<td nowrap>
					<span onclick="newFolder()" style="cursor:pointer;" <%=sFolderAdmin%>><u><span name="txtLang" id="txtLang">New&nbsp;Folder</span></u></span>&nbsp;
					<span onclick="deleteFolder()" style="cursor:pointer;" <%=sFolderAdmin%>><u><span name="txtLang" id="txtLang">Del&nbsp;Folder</span></u></span>
				</td>
