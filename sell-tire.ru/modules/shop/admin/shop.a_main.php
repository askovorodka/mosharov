...
/*
$id = $tree->clear();
$tree->insert($id, array("name" => "Шины", "url" => "shini"));
$tree->insert($id, array("name" => "Диски", "url" => "diski"));
*/
...
if ($action == 'text_generate')
{
	...
	{
		$parent = $shop->getParent($model, 2);
		$text = str_replace("{brand_name}", $parent['name'], str_replace("{model_name}", $model['name'], CATEGORY_TEXT_TEMPLATE) );
								...
								foreach ($matches[$key] as $key2=>$val2)
								{
									...
									if (preg_match("/\[(.*)\]/", $matches[$key][$key2], $str) )
									{
										if (isset($str[1]))
										{
											...
											$word = $array[$random];
											$text = str_replace($matches[$key][$key2], $word, $text);
	    ...
	    $db->query("update fw_catalogue set text='{$text}' where id='{$model['id']}'");
	...
	if ($check) {
		...
		if (@$file_name!='') {
			...
			if (move_uploaded_file($tmp, BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext)) {
				...
				//превьюшка
				Image::image_resize(BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext,BASE_PATH.'/images/scat-'.$id.'.'.$ext,86,98);
	...
	if ($check || $id=="1") {
		...
		if (@$file_name!='') {
			if (move_uploaded_file($tmp, BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext)) {
				...
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/'.$image_name,BASE_PATH.'/uploaded_files/shop_images/ccat'.$image_name,86,98, false);
