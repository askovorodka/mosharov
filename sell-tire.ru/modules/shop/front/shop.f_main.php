...
if (!empty($_POST['fast_order']) && $_POST['fast_order'] == 1)
{
	...
	else
	{
		...
		//Mail::send_mail("shini-kypit@ya.ru","info@sell-tire.ru","Быстрый заказ интернет магазине #{$order_id}",$body,'','html','standard','Windows-1251');
		//Mail::send_mail("snegovik3@gmail.com","info@sell-tire.ru","Быстрый заказ интернет магазине #{$order_id}",$body,'','html','standard','Windows-1251');
		Mail::send_mail("ai@avtozeon.ru","info@sell-tire.ru","Быстрый заказ интернет магазине #{$order_id}",$body,'','html','standard','Windows-1251');
		Mail::send_mail("rifshina@mail.ru","info@sell-tire.ru","Быстрый заказ интернет магазине #{$order_id}",$body,'','html','standard','Windows-1251');
	...
	CASE ($url[$n]=='submit' && $url[$n-1]=='basket' && count($url)==3):
		...
		if (isset($_POST['submit_order'])) 
		{
			...
			else
			{
				...
				//Mail::send_mail($user['mail'],"ai@avtozeon.ru,avtozeon@gmail.com,rifshina@mail.ru,itire@ya.ru","Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
				Mail::send_mail($user['mail'],"shini-kypit@ya.ru","Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
				...
				//Mail::send_mail("shini-kypit@ya.ru",$user['mail'],"Новый заказ в интернет магазине #{$order_id}",$admin_body,'','html','standard','Windows-1251');
