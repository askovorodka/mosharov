...
class Shop extends db {
	...
	{
		$result = $this->db->get_single("select image from fw_catalogue where id='$id' ");
	...
	//популярные шины
	function getTopProducts($limit = 6, $type = null)
	{
		
		if (!empty($type))
			$where = " and d.id = " . intval($type);
		else
			$where = "";
		
		$result = $this->db->get_all("select a.*, b.image, c.name as manufacture_name 
			from fw_products as a 
			inner join fw_catalogue as b on a.parent=b.id
			inner join fw_catalogue as c on c.param_left < b.param_left 
			and c.param_right > b.param_right and c.param_level = 2
			inner join fw_catalogue as d on d.param_left < b.param_left 
			and d.param_right > b.param_right and d.param_level = 1 $where
			where a.status='1' and a.hit='1' order by rand() ");
		
		if ($result && count($result) > 0)
		{
			foreach ($result as $key=>$val)
			{
				$result[$key]['full_url'] = $this->getFullUrlProduct($val['id'], "katalog");
			}
			return $result;
		}
		else 
