...
//echo iconv('utf-8','windows-1251',"test");
...
SWITCH (TRUE){
  ...
  CASE ($url[$n]=='orders' && count($url)==2):
    ...
    $orders_list=$db->get_all("SELECT *, (total_price + order_price) dostavka_sum  
    FROM fw_orders WHERE user='".$_SESSION['fw_user']['id']."' 
    ...
    foreach ($orders_list as $key=>$val)
    {
    	...
    	if ($orders_list[$key]['dostavka'] > 2)
    		$orders_list[$key]['company'] = $companies[$orders_list[$key]['dostavka']];
