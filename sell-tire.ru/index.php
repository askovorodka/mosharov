...
{
	header("Location: http://sell-tire.ru" . $_SERVER['REQUEST_URI']);
	die();
}
...
if ($user_id = $users->is_auth_user())
	$smarty->assign('user', $users->get_user($user_id));
...
$css[] = BASE_URL."/templates/style_13_04_12.css";
...
$js[]=BASE_URL."/javascript/tools_16-04-12.js";
...
//рекомендуем
$shop = new Shop($db);
$smarty->assign('top_tires', $shop->getTopProducts(6, TIRES_ID));
$smarty->assign('top_disk', $shop->getTopProducts(6, DISK_ID));
...
elseif ($_SERVER['REQUEST_URI'] == '/last_import_time/')
{
	echo IMPORT_LAST_TIME; die();
}
else {
...
if ($page_found) header("HTTP/1.0 200 OK");
  ...
  $smarty->assign("page_title","Ошибка 404. Cтраница не найдена.");
