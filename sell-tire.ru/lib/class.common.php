...
class Common {
	...
	function generate_main_menu($level=0) {
		...
		$res=$db->get_all("SELECT * FROM fw_tree WHERE $level_str status='1' and url not in ('kabinet', 'bistriy_zakaz_ok') ORDER BY param_left");

		$content=Common::generate_menu($res, $res[0]['param_left'], $res[0]['param_right']);

		$modules = array('shop','site_map','news');
