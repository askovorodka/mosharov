...
function InnovaEditor(oName)
    ...
    this.btnAbsolute=true;this.btnPasteWord=true;this.btnLine=true;this.btnTables=true;this.btnPhoto=true;this.btnMail=true;
...
function RENDER(sPreloadHTML)
    ...
    for(var i=0;i<this.buttonMap.length;i++)
        ...
        switch(sButtonName)
			...
			case "PasteWord":
				...
				break;
			case "Tables":
				if(this.btnTables)sHTMLIcons+=this.writeIconStandard("btnTables"+this.oName,"modelessDialogShow('"+this.scriptPath+"../../../../admin/index.php?mod=tables&action=mini_browser',600,480)","btnTables.gif",getText("No Tables"));
				break;
			case "Photo":
				if(this.btnPhoto)sHTMLIcons+=this.writeIconStandard("btnPhoto"+this.oName,"window.open('../../admin/index.php?mod=photoalbum&action=mini_browser','Site_Path','width=600,height=400,resizable=yes')","btnPhoto.gif",getText("No Photos"));
				break;
			case "Mail":
				if(this.btnMail)sHTMLIcons+=this.writeIconStandard("btnMail"+this.oName,"window.open('../../../admin/index.php?mod=forms&action=mini_browser','Site_Path','width=600,height=400,resizable=yes')","btnMail.gif",getText("No Mail"));
				break;
            case "Bold":
	...
	if(this.btnTables)this.arrElm[33]=this.getElm("btnTable");
	if(this.btnPhoto)this.arrElm[33]=this.getElm("btnPhoto");
	if(this.btnMail)this.arrElm[33]=this.getElm("btnMail");
    }
...
function getHTMLBody()
    {
    var oEditor=document.getElementById("idContent"+this.oName).contentWindow;
    sHTML=unescape(oEditor.document.body.innerHTML);
    sHTML=String(sHTML).replace(/ contentEditable=true/g,"");
    sHTML = String(sHTML).replace(/\<PARAM NAME=\"Play\" VALUE=\"0\">/ig,"<PARAM NAME=\"Play\" VALUE=\"-1\">");
