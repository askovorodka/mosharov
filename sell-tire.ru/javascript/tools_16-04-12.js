...
$(document).ready( function(){
	...
	$("a.tabs").click(function(){
		...
		$("a.tabs").parent().removeClass("fthis");
		$(this).parent().addClass("fthis");
		return false;
	});
	...
	function _change_fast_order()
	{
		...
		if (parseInt(product_id) > 0 && parseInt(product_count) > 0)
		{
			...
			$.post('http://' + location.hostname + '/katalog/', 
					{ajax:1, 
	...
	var register_email_validate = $("form#form_order").validate(
			{
				...
				rules:
				{
					email :	{required: true, email : true, remote : 'http://' + location.hostname + '/katalog/ajax/checkemail/'},
	...
	$("FORM#form_order_single").submit(function(){
		if (form_order_single_validate.form())
		{
			...
			$.post('http://' + location.hostname + '/katalog/basket/add/', 
