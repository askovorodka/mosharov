...
def get_categories():
    ...
    for item in categories:
        category = list(item)
        full_url = get_category_full_url(category, 'katalog')
...
def get_category_full_url(category, module_name = 'katalog'):
    url = str(category[4]) + '/'
...
def get_products():
    ...
    for item in products:
        ...
        full_url_product = DOMAIN + get_category_full_url(category, 'katalog') + str(pr[0]) + "/"
...
for product in products:
    ...
    #f.write("<local_delivery_cost>200</local_delivery_cost>")
    ...
    if product[17] != "0.00" and product[17] > 0:
        f.write("<param name=\"Ширина\">%s</param>" % product[17])
        f.write("<param name=\"Диаметр\">%s</param>" % product[18])
        f.write("<param name=\"Dia\">%s</param>" % product[23])
        f.write("<param name=\"Крепеж\">%s</param>" % product[19])
        f.write("<param name=\"PCD\">%s</param>" % product[20])
        f.write("<param name=\"PCD (двойной)\">%s</param>" % product[21])
        f.write("<param name=\"ET\">%s</param>" % product[22])
        f.write("<param name=\"Цвет\">%s</param>" % product[24].decode('cp1251').encode('utf-8'))
    else:
        f.write("<param name=\"Ширина\">%s</param>" % product[27])
        f.write("<param name=\"Профиль\">%s</param>" % product[28])
        f.write("<param name=\"Диаметр\">%s</param>" % product[29])
        if product[30] != "":
