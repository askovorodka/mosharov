...
$news=$db->get_all("SELECT * FROM fw_news WHERE status='1' and date_format(from_unixtime(publish_date),'%Y-%m-%d') <= curdate() ORDER BY publish_date DESC limit 3");
...
if ($shop_menu)
{
	...
	{
		$shop_menu[$key]['full_url'] = $shop->getFullUrlCategory($val['id'], "rents");
