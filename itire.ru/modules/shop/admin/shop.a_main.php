...
{
	//параметры для goodrims.ru
	$db->query("update fw_conf set conf_value='{$_POST['goodrims_tire_koef']}' where conf_key='GOODRIMS_TIRE_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['goodrims_disk_koef']}' where conf_key='GOODRIMS_DISK_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['goodrims_tire_round_koef']}' where conf_key='GOODRIMS_TIRE_ROUND_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['goodrims_disk_round_koef']}' where conf_key='GOODRIMS_DISK_ROUND_KOEF' ");

	if (!empty($_POST['goodrims_enable']))
		$g_enable = 1;
	else
		$g_enable = 0;
	$db->query("update fw_conf set conf_value='{$g_enable}' where conf_key='GOODRIMS_ENABLE' ");

	//параметры для sell-tire.ru
	$db->query("update fw_conf set conf_value='{$_POST['selltire_tire_koef']}' where conf_key='SELLTIRE_TIRE_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['selltire_disk_koef']}' where conf_key='SELLTIRE_DISK_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['selltire_tire_round_koef']}' where conf_key='SELLTIRE_TIRE_ROUND_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['selltire_disk_round_koef']}' where conf_key='SELLTIRE_DISK_ROUND_KOEF' ");
	
	if (!empty($_POST['selltire_enable']))
		$s_enable = 1;
	else
		$s_enable = 0;
	$db->query("update fw_conf set conf_value='{$s_enable}' where conf_key='SELLTIRE_ENABLE' ");

	
	//параметры для cccp-shina.ru
	$db->query("update fw_conf set conf_value='{$_POST['cccp_tire_koef']}' where conf_key='CCCP_TIRE_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['cccp_disk_koef']}' where conf_key='CCCP_DISK_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['cccp_tire_round_koef']}' where conf_key='CCCP_TIRE_ROUND_KOEF' ");
	$db->query("update fw_conf set conf_value='{$_POST['cccp_disk_round_koef']}' where conf_key='CCCP_DISK_ROUND_KOEF' ");
	
	if (!empty($_POST['cccp_enable']))
		$c_enable = 1;
	else
		$c_enable = 0;
	$db->query("update fw_conf set conf_value='{$c_enable}' where conf_key='CCCP_ENABLE' ");
...
if (isset($_POST['submit_edit_product'])) {
	...
	if (intval($tire_width) > 0)
		curl_setopt($curl, CURLOPT_POSTFIELDS, array(
			"name" => $name, 
			"price" => $price, 
			"tire_sklad" => $tire_sklad, 
			"type" => "tire", 
			"goodrims_tire_koef" => GOODRIMS_TIRE_KOEF, 
			"goodrims_tire_round_koef" => GOODRIMS_TIRE_ROUND_KOEF));
	
	elseif (intval($disk_width) > 0)
		curl_setopt($curl, CURLOPT_POSTFIELDS, array(
		"name" => $name,
		"price" => $price,
		"disk_sklad" => $disk_sklad,
		"type" => "disk",
		"goodrims_disk_koef" => GOODRIMS_DISK_KOEF,
		"goodrims_disk_round_koef" => GOODRIMS_DISK_ROUND_KOEF));
	...
	if (intval($tire_width) > 0)
		curl_setopt($curl, CURLOPT_POSTFIELDS, array(
				...
				"tire_sklad" => $tire_sklad,
				"type" => "tire",
				"selltire_tire_koef" => SELLTIRE_TIRE_KOEF,
				"selltire_tire_round_koef" => SELLTIRE_TIRE_ROUND_KOEF));
	...
	curl_setopt($curl, CURLOPT_POSTFIELDS, array(
			...
			"disk_sklad" => $disk_sklad,
			"type" => "disk",
			"selltire_disk_koef" => SELLTIRE_DISK_KOEF,
			"selltire_disk_round_koef" => SELLTIRE_DISK_ROUND_KOEF));
	...
	if (intval($tire_width) > 0)
		curl_setopt($curl, CURLOPT_POSTFIELDS, array(
				...
				"tire_sklad" => $tire_sklad,
				"type" => "tire",
				"cccp_tire_koef" => CCCP_TIRE_KOEF,
				"cccp_tire_round_koef" => CCCP_TIRE_ROUND_KOEF));
	...
	curl_setopt($curl, CURLOPT_POSTFIELDS, array(
			...
			"disk_sklad" => $disk_sklad,
			"type" => "disk",
			"cccp_disk_koef" => CCCP_DISK_KOEF,
			"cccp_disk_round_koef" => CCCP_DISK_ROUND_KOEF));
