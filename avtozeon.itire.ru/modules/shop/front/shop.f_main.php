...
if  ($main_module=='on')
{
	...
	if (isset($_GET['update_price_from_itire']))
	{
		...
		else
		{
			...
			if ($_POST['type'] == 'tire')
			{
				...
				$tire_koef = $_POST['goodrims_tire_koef'];
				$tire_round_koef = $_POST['goodrims_tire_round_koef'];
			...
			elseif ($_POST['type'] == 'disk')
			{
				...
				$disk_koef = $_POST['goodrims_disk_koef'];
				$disk_round_koef = $_POST['goodrims_disk_round_koef'];
	...
	if (!empty($_POST['fast_order']) && $_POST['fast_order'] == 1)
	{
		...
		{
			if ($error_count)
			$smarty->assign('error_fast_order', "Превышено количество товаров, на складе осталось только: ". $count_true . " шт.");

			if ($errors)
			$smarty->assign('error_fast_order', "Заполните необходимые поля для быстрого заказа");
		...
		else
		{
			...
			insert_date,
			dostavka,
			order_price, fast_order, comments)
			VALUES(
			'".$name."',
			'".$phone."',
			'".$address."',
	...
	SWITCH (TRUE) {
		...
		/*CASE ($url[$n] == 'top_products'):
			...
			//$top_products = $shop->getTopProducts(1000);

			$top_disk = $shop->getTopProducts(1000, DISK_ID);
			$top_tires = $shop->getTopProducts(1000, TIRES_ID);

			$smarty->assign('top_disk', $top_disk);
			$smarty->assign('top_tires', $top_tires);
			$page_title = "Специальные предложения ШИНЫ-ПРОДАЖА.РФ";
		...
		CASE (preg_match("/^([0-9]+)$/",$url[$n]) && $url[$n-1]=='delete' && $url[$n-2]=='basket' && count($url)==4):
			...
			$location=$_SERVER['HTTP_REFERER'];
			//header("Location: $location");
			header("Location: /catalog/basket/?ver=" . sha1(rand(1000,10000)));
			die();
		...
		CASE ( ($url[$n]=='basket' && count($url)==2) || (preg_match("/\?ver=(.+)$/",$url[$n]) && $url[$n-1] == 'basket' && count($url)==3) ):
							...
							if ($product)
							{
								...
								if (!empty($product['tire_width']))
								{
									if ($product['tire_sklad'] < $_SESSION['fw_basket'][$i]['number'])
									{
										...
										$error_msg[] = 'Максимальное количество ' . $product['name'] . ' на складе: ' . $product['tire_sklad'] . ' шт.';
										$count_error = true;
									}
								}
								elseif (!empty($product['disk_width']))
								{
									if ($product['disk_sklad'] < $_SESSION['fw_basket'][$i]['number'])
									{
										...
										$error_msg[] = 'Максимальное количество ' . $product['name'] . ' на складе: ' . $product['disk_sklad'] . ' шт.';
										$count_error = true;
									}
								}
							}
			...
			if (count($_SESSION['fw_basket'])>0) {
				...
				foreach($sess['fw_basket'] as $key=>$val){
					...
					$sess['fw_basket'][$key]['full_url'] = DOMAIN . $shop->getFullUrlProduct($sess['fw_basket'][$key]['id'], 'catalog');
		...
		CASE ($url[$n]=='final' && $url[$n-1]=='basket' && count($url)==3):
			...
			$navigation[]=array("url" => 'basket',"title" => 'Благодарим вас за заказ');
		...
		CASE ($url[$n]=='submit' && $url[$n-1]=='basket' && count($url)==3):
			...
			if (isset($_POST['submit_order']))
			{
				...
				else
				{
					...
					Mail::send_mail($user['mail'],"ashmitz@yandex.ru","Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
					...
					Mail::send_mail("aschmitz@ya.ru",$user['mail'],"Новый заказ в интернет магазине #{$order_id}",$admin_body,'','html','standard','Windows-1251');
		...
		CASE ($url[1] == 'search'):
			...
			if ($_GET['filter_type'] == 'disk')
			{
				...
				$navigation[]=array("url" => 'search',"title" => 'Результат поиска дисков');
			}
			elseif ($_GET['filter_type'] == 'tire')
			{
				$where[] = " a.tire_width > 0 ";
				$navigation[]=array("url" => 'search',"title" => 'Результат поиска шин');
			}
		...
		CASE (
		(count($url)==2 && preg_match("/\?search_product=(.+)$/",$url[$n])) or
		(count($url)==2 && preg_match("/\?search_product=(.+)&page=([1-9]+)$/",$url[$n])) or
		(count($url)==2 && preg_match("/\?x=([0-9]+)&y=([0-9]+)&search_product=(.+)$/",$url[$n]))
		):

		$navigation[]=array("url" => 'search',"title" => 'Результаты поиска');
		...
		$page_title = "Результаты поиска ШИНЫ-ПРОДАЖА.РФ" ;
				...
				if ($cat_list[$f]['full_url']==$url_to_check) {
					...
					for ($c=0;$c<count($cat_list);$c++) {
						...
						if ($cat_list[$c]['param_left'] < $cat_content['param_left'] && $cat_list[$c]['param_right'] > $cat_content['param_right'] && $cat_list[$c]['param_level'] == $cat_content['param_level']-1) {
							...
							//находим список моделей для бренда
