...
if (isset($_POST['send_admin_mail']))
{
	Mail::send_mail($_POST['to'],$_POST['from'],$_POST['subject'],$_POST['message'],'','text','standard','Windows-1251');
	...
	if ($check) {
		...
		if (@$file_name!='') {
			...
			if (move_uploaded_file($tmp, BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext)) {
				...
				//превьюшка
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext,BASE_PATH.'/images/scat-'.$id.'.'.$ext,156,45, false);
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext,BASE_PATH.'/images/cat-'.$id.'.'.$ext,215,236, false);
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext,BASE_PATH.'/images/pcat-'.$id.'.'.$ext,139,100, false);
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/-'.$id.'.'.$ext,BASE_PATH.'/images/bcat-'.$id.'.'.$ext,$details['width'],$details['height'], false);
				Image::resize(BASE_PATH.'/uploaded_files/shop_images/'.$id.'.'.$ext,BASE_PATH.'/uploaded_files/shop_images/ccat'.$id.'.'.$ext,98,81, false);
	...
	CASE ($action=='products_list'):
		...
		if ((isset($sort_cat)) or (isset($_GET['search']) && $_GET['search']>'1')  or  (isset($sort_type))) {
			...
			if (isset($_GET['search'])) {
				$temp_cond[]="(name LIKE '%".$_GET['search']."%'  OR article='{$_GET['search']}')";
...
/****************************************************************/
	...
	CASE ($action=='order_details'):
		...
		if ($order_info['dostavka'] > 2)
			$order_info['company_order'] = $companies[$order_info['dostavka']];
		
		//$smarty->assign("user_info",$user_info);
		$smarty->assign("order_info",$order_info);
		
		if (!empty($order_info['user']))
		{
			$user = $users->get_user(intval($order_info['user']));
