...
if (isset($_POST['submit_add_news'])) {
	...
	if ($check) {
		...
		$result=$db->query("INSERT INTO fw_news (title,small_text,text,status,publish_date, cat_id) VALUES('$title','$small_text','$text','$status','".time()."', '{$cat_id}')");
		if ($result) { 
...
if (isset($_POST['submit_edit_news'])) {
	...
	$cat_id = intval($_POST['edit_news_cat']);
	...
	if ($check) {
		...
		$result=$db->query("UPDATE fw_news SET title='$title',small_text='$small_text',text='$text',status='$status',publish_date='$time',image='$image', cat_id='{$cat_id}' WHERE id='$id'");
	}
}
