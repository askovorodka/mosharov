...
if  ($main_module=='on')
{
	...
	SWITCH (TRUE) {
				...
				else
				{
					...
					elseif (!empty($_POST['password']))
					{
						...
						else
						{
							...
							$error_register = true;
							echo "Пользователь с тами email уже зарегистрирован";
					...
					Mail::send_mail($user['mail'],"goodrims@yandex.ru","Новый заказ в интернет магазине",$body,'','html','standard','Windows-1251');
		...
		CASE ($url[1] == 'search'):
			...
			$smarty->assign("products",$products);
			$page_found=true;
			$page_title = "Результаты писока ШИНЫ-ПРОДАЖА.РФ";
