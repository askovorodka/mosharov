...
//					}
	...
	// Старая процедура добавления в корзину
	CASE (preg_match("/^([0-9]+)$/",$url[$n]) && $url[$n-1]=='add' && $url[$n-2]=='basket' && count($url)==4):

		$number_found=false;

		$product=$db->get_single("SELECT id,parent,name,price,sale FROM fw_products WHERE id='".$url[$n]."' AND status='1'");

        //$product['price']=number_format(($product['price'] * $cur_admin['kurs'])/$cur_site['kurs'],2);
	...
	CASE (@$url[$n-1]=='basket_add' && @preg_match("/^\?PHPSESSID=[a-z0-9]+\&product_id=([0-9]+)\&product_number=[0-9]+\&[0-9]+$/",$url[$n]) && isset($_GET['product_id']) && count($url)==3):
		...
		$_RESULT = array(
		  ...
		  //"basket_total"=>sprintf("%.2f",$basket_total),
	...
	//CASE (@$url[$n-1]=='search_product' && @preg_match("/^\?PHPSESSID=[a-z0-9]+\&keyword=([a-z_0-9]+)\&[0-9]+$/",$url[$n]) && isset($_GET['query']) && count($url)==3):
	...
	CASE ($url[$n]=='basket' && count($url)==2):
		...
		//foreach ($_SESSION['fw_basket'][0] as $key=>$val)
		//	echo $key."=".$val."<br>";
		...
		if (!$page_found) {
			...
			if (preg_match("/^([0-9]+)$/",$url[$n])) {
				...
				if (strlen(trim($next_product['id']))>0)
					...
					$tmp=explode("##|##",$product_content['properties']);
					$product_content['properties']=array();
					foreach ($tmp as $val => $k) {
						if (substr_count($k,"||#||")>0) {
							$tmp2=explode("||#||",$k);
							$product_content['properties'][]=$tmp2;
							if (substr_count($product_content['properties'][$val][3],"\n")>0) $product_content['properties'][$val][3]=explode("\n",$product_content['properties'][$val][3]);
						}
					}
