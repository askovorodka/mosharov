...
class Import
{
	...
	function getImportTypeByFile($file)
	{
		if (file_exists($this->csv_dir . $file))
		{
			...
			$f = fopen($this->csv_dir . $file, "rt") or die("Ошибка!");
			...
			foreach ($this->data as $val)
			{
				...
				else 
				{
					$this->tree->insert($parent['id'], array(
						'name' => $brand_name,
						'url' => $this->string->translit(strtolower($brand_name)),
						'text' => '',
						'status' => '1',
						'title' => $brand_name,
				...
				if ($brand_id)
				{
					...
					else
					{
						$this->tree->insert($brand['id'], array(
							'name' => $val[2],
							'url' => $this->string->translit(strtolower($val[2])),
	...
	{
		//определяем сезон
		if ($fields['tire_season'] == 'зимняя')
		{
			$season = 1;
		}
		elseif ($fields['tire_season'] == 'летняя')
		{
			$season = 2;
		}
		else 
